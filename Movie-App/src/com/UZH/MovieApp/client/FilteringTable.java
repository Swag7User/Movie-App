...
public class FilteringTable extends Composite {
	...
	// checkboxes to determine if field is activated
	...
	final Button sendButton2 = new Button("Check Database");
	...
	public Widget createFilterTable() {
		...
		// Start off with everythign greyed out
...
//		});
		...
		// Hook up a handler to find out when they're clicked clicked.
		activateMasterMode.addClickHandler(new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				boolean checked = ((CheckBox) event.getSource()).getValue();
				if (checked) {
					...
					final Button closeWarning = new Button("Close");
					final Label warningLabel = new Label();
					warning.setText("RPC to DB");
					warning.setAnimationEnabled(false);
					...
					warningPanel.add(closeWarning);
					warning.setWidget(warningPanel);
					
					closeWarning.addClickHandler(new ClickHandler() {
						public void onClick(ClickEvent event) {
							warning.hide();
						}
					});
					...
					warningLabel.addStyleName("serverResponseLabelError");
					warning.center();
					closeWarning.setFocus(true);
	...
	//	filteringTable.setWidget(17, 3, limitField);
		...
		FlexTable goButtonTable = new FlexTable();
		goButtonTable.setWidget(0, 0, sendButton2);

		// seperate table for master mode
		FlexTable masterTable = new FlexTable(); 
		masterTable.setWidget(0, 0, activateMasterMode);
		masterTable.setText(0, 1, "SQL:");
		masterTable.setWidget(0, 2, masterField);
		masterTable.setWidget(0, 3, masterSendButton);
		...
		filteringPanel.add(goButtonTable);
